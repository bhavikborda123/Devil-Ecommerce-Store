...
class Product {
  ...
  async postAddProduct(req, res) {
    ...
    // Validate Images
      ...
      Product.deleteImages(images, "file");
      return res.json({ error: "At least need to provide 2 images" });
    } else {
      try {
        let allImages = [];
  ...
  async postEditProduct(req, res) {
    ...
    else if (editImages && editImages.length > 0 && editImages.length < 2) {
  ...
  async updateStock(req, res) {
    ...
    try {
      await Promise.all(
        productData.map(async (product) => {
          ...
          let updatedProduct = await productModel.findByIdAndUpdate(id, {
            $inc: { pQuantity: -quantity, pSold: +quantity },
          });
          if (!updatedProduct) {
      ...
      return res.json({ success: "Stock updated successfully" });
    } catch (err) {
      return res.json({ error: err.message });
    }
  }
