...
const ProductDetailsSection = (props) => {
  ...
  const [quantity, setQuantity] = useState(1); // Increase and decrease quantity state
  // eslint-disable-next-line
  const [alert, setAlertq] = useState(false); // Alert when quantity greater than stock
  ...
  const fetchCartProduct = async () => {
    try {
      ...
      if (responseData && responseData.Products) {
        layoutDispatch({ type: "cartProduct", payload: responseData.Products }); // Layout context Cart product fetch and dispatch
      }
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <Fragment>
      <Submenu
        value={{
          categoryId: sProduct.pCategory ? sProduct.pCategory._id : null,
          product: sProduct.pName,
          category: sProduct.pCategory ? sProduct.pCategory.cName : null,
        }}
      />
        ...
        <div className="grid grid-cols-2 md:grid-cols-12">
          ...
          <div className="col-span-2 mt-8 md:mt-0 md:col-span-4 md:ml-6 lg:ml-12">
            ...
            <div className="my-4 md:my-6">
              ...
              <div
                className={`flex justify-between items-center px-4 py-2 border ${+quantity === +sProduct.pQuantity && "border-red-500"
                  }`}
              >
                <div
                  className={`${quantity === sProduct.pQuantity && "text-red-500"
                    }`}
                >
                  Quantity
                </div>
              ...
              {/* In cart and out of stock button */}
              {sProduct.pQuantity !== 0 ? (
                <Fragment>
                  {layoutData.inCart !== null &&
              ...
              {/* In cart and out of stock button End */}
            </div>
          </div>
        </div>
      </section>
