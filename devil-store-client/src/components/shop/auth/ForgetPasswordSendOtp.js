...
export const ForgetPasswordSendOtp = () => {
  ...
  const sendOtp = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      if (response.message) {
        history.push('/reset-password/verify-otp');
      }
      else {
        ...
        <Container>
          <div className='d-flex justify-content-center' style={{ marginTop: "150px" }}>
            <Card className='shadow-lg' style={{ width: '30rem', border: 'none' }}>
              <h2 className='d-flex justify-content-center text-black text-success mt-4 mb-4'>Forget Password ? </h2>
              <Card.Body>
                ...
                />
                <br />
                <Button className='bg-black' variant="success" style={{ width: "100%" }} type='submit' disabled={loading}>  {loading ? 'Loading...' : 'Reset Password'}</Button>
